...
function createDiary(obj) {
  ...
  return fetch(apiLink + "/api/v1/diary", {
    ...
    .then(response => {
      setTimeout(() => {
        bus.publish("DiaryCreated", {
          message: "Diary '" + obj.diaryName + "' created",
...
function getDiaryName() {
  return fetch(
    apiLink + "/api/v1/diary/" + localStorage.getItem("profileName"),
    {
      method: "GET"
    }
  )
